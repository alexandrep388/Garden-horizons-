...
-- Attend que RUN!! apparaît dans le GUI (= devient brainrot)
local function waitForRun(timeoutSeconds)
    timeoutSeconds = timeoutSeconds or 60
    local gui = player.PlayerGui
    local startTime = tick()

    -- Attend que le HUD soit chargé
    local hud = gui:WaitForChild("HUD", 10)
    if not hud then return false end

    local runFrame = hud:WaitForChild("RunTopFrame", 10)
    if not runFrame then return false end

    local runObj = runFrame:WaitForChild("RUN", 10)
    if not runObj then return false end

    local title = runObj:WaitForChild("Title", 10)
    if not title then return false end

    while tick() - startTime < timeoutSeconds do
        if title.Text == "RUN!!" then
            return true
        end
        task.wait(0.05)
    end
...
Loading.Sidebar:AddLabel("Version : v2.0.0")
task.wait(0.8)
...
local Window = Library:CreateWindow({
    Title = "GD Studio",
    Footer = "Être un Lucky Block • v2.0.0",
    Icon = "zap",
    NotifySide = "Right",
...
FarmLeft:AddSlider("FarmDelay", {
    ...
    Default = 2, Min = 1, Max = 15, Rounding = 0, Suffix = "s",
})
...
FarmLeft:AddToggle("AutoFarm", {
    Text = "Auto Farm",
    Default = false,
    Tooltip = "Farm automatique — détecte RUN!! via GUI",
})
...
Toggles.AutoFarm:OnChanged(function(state)
    ...
    task.spawn(function()
        while Toggles.AutoFarm.Value do
            ...
            -- ÉTAPE 1 : TP CatchPos pour se faire attraper
            StatusLabel:SetText("Status : TP zone catch...")
            tpTo(CatchPos)

            -- ÉTAPE 2 : Attend que "RUN!!" apparaît = on est devenu brainrot
            StatusLabel:SetText("Status : Attente brainrot...")
            local caught = waitForRun(60)
            ...
            -- ÉTAPE 3 : RUN!! détecté → TP SafePos immédiat
            StatusLabel:SetText("Status : Brainrot détecté ! Escape...")
...
Toggles.AutoEscape:OnChanged(function(state)
    ...
    task.spawn(function()
        ...
        local title = hud:WaitForChild("RunTopFrame", 10)
            :WaitForChild("RUN", 10)
...
Library:Notify({
    Title = "GD Studio",
    Content = "Bienvenue " .. player.Name .. " ! v2.0.0",
    Duration = 3,
})
    :FindFirstChild("sleitnick_knit@1.7.0").knit.Services
local RFBrainrot = RFBase.ContainerService.RF.UpgradeBrainrot
